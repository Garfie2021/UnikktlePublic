...
{
    public static class 抽出Bing
    {
        private static int count = 0;

        public static void Exec(SqlConnection cn, DateTime now)
        {
            var list = DB.Collect.SP_HtmlParseBing.Select_State0(cn);
            count = list.Count();

            foreach (var htmlParseRow in list)
            {
                // HTMLタグ除去
                var 抽出文List = ExtractBing.必要な文のみ抽出(htmlParseRow);

                // Bing検索結果のTop20をDBに登録する。
                foreach (var 抽出文 in 抽出文List)
                {
                    ...
                    if (言語判定 == 言語判定結果.日本語)
                    {
                        ...
                        foreach (var keywordRow in keywordRowList)
                        {
                            ...
                            DB.Collect.BulkCopy_CollectTargetKeyword_Bing_Tmp.Add(
                                htmlParseRow.SearchKeywordNo,
                                htmlParseRow.SearchDate,
                                抽出文.SearchResultNo,
                                keywordRow2.No, 
                    ...
                    DB.Collect.BulkCopy_ExtractBing.Add(
                        htmlParseRow.SearchKeywordNo,
                        htmlParseRow.SearchDate,
                        抽出文.SearchResultNo,
                        抽出文.関連キーワード,
            ...
            DB.Collect.BulkCopy_ExtractBing.Flush(AppSetting.ConnectionString_UnikktleCollect);
            ...
            {
                DB.Collect.SP_HtmlParseBing.Update_State(cn, row, State.解析済み);
            }

            ログ.ログ書き出し($"抽出Bing End. Count{count}");
        }
