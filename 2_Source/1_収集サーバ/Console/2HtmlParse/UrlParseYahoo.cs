...
{
    public static class UrlParseYahoo
    {
        //private static int count = 0;

        public static void Exec(SqlConnection cn, DateTime now)
        {
            var dataTable = DB.Collect.SP_CollectYahoo.Select_CutoutStateUrl0(cn);
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                DB.Collect.SP_CollectYahoo.Update_CutoutStateUrl(cn,
                    (long)row["SearchKeywordNo"],
                    (DateTime)row["SearchDate"],
                    State.解析済み);
            }

            ログ.ログ書き出し($"Yahoo Url Cnt:{dataTable.Rows.Count}");
        }
