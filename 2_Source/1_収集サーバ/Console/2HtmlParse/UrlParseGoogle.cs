...
{
    public static class UrlParseGoogle
    {
        //private static int count = 0;

        public static void Exec(SqlConnection cn, DateTime now)
        {
            var dataTable = DB.Collect.SP_CollectGoogle.Select_CutoutStateUrl0(cn);
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                DB.Collect.SP_CollectGoogle.Update_CutoutStateUrl(cn,
                    (long)row["SearchKeywordNo"],
                    (DateTime)row["SearchDate"],
                    State.解析済み);
            }

            ログ.ログ書き出し($"Google Url Cnt:{dataTable.Rows.Count}");
        }
