...
namespace DB.Collect
{
    public static class SP_HtmlParseYahoo
    {
        ...
        public static List<HtmlParseYahooRow> Select_State0(SqlConnection cn)
        {
            ...
            // DataTableをListに入れ替え
            var list = new List<HtmlParseYahooRow>();
            foreach (DataRow row in table.Rows)
            {
                list.Add(new HtmlParseYahooRow()
                {
        ...
        public static void Update_State(SqlConnection cn,
            HtmlParseYahooRow HtmlParseYahooRow, State State)
        {
            try
            {
                cmdUpdate_State.Connection = cn;
                cmdUpdate_State.Parameters["SearchKeywordNo"].Value = HtmlParseYahooRow.SearchKeywordNo;
                cmdUpdate_State.Parameters["SearchDate"].Value = HtmlParseYahooRow.SearchDate;
            ...
            {
                ログ.ログ書き出し(ex);
                ログ.ログ書き出し("SearchKeywordNo: " + HtmlParseYahooRow.SearchKeywordNo +
                    "\r\n　SearchDate: " + HtmlParseYahooRow.SearchDate.ToString());
                throw;
            }
        }
