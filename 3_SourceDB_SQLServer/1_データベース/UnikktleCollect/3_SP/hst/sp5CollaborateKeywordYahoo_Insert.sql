...
/*
*/
IF OBJECT_ID(N'[hst].[sp5CollaborateKeywordYahoo_Insert]', N'P') IS NOT NULL
	DROP PROCEDURE [hst].[sp5CollaborateKeywordYahoo_Insert] ;
GO
...
-- 再計算ストアド
CREATE PROCEDURE [hst].[sp5CollaborateKeywordYahoo_Insert]
AS
BEGIN
	...
	--BEGIN
		...
		-- 古いデータは削除
		DELETE FROM [hst].[t5CollaborateKeyword_Yahoo]
		--WHERE [KeywordNo_元] = @No;


		-- キーワードの新しい関連をInsert
		INSERT INTO [hst].[t5CollaborateKeyword_Yahoo]
		(
			[KeywordNo_元],
			[KeywordNo_先],
			[同時出現ドキュメント数]
		...
		--	FROM [hst].[t4CollectTargetKeyword_Yahoo] AS 元 INNER JOIN [hst].[t4CollectTargetKeyword_Yahoo] AS 先
